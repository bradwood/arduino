...
void SFRGBLEDMatrix::show() {
  ...
  for(uint16_t p=0;p<frameBuffSize;p++){
    #if defined DEBUG
		Serial.println("sending SPI byte...");
	#endif
...
/*
void SFRGBLEDMatrix::printweek(const Color color, const int xOffset, const int yOffset, const char c)
{
	  prog_uint16_t *coffset;
	  prog_uchar **line;
      
	  coffset=coffset_week;
	  line=line_week;
...
//NOTE: y1 must be > y0 -- NO error checking here....
void SFRGBLEDMatrix::line_vert(const Color color, const int x0, const int y0, const int y1)
{
	for(int i = y0; i <= y1; ++i)
	{
		paintPixel(color,x0,i);
	}	
}
