/*
 * Accelerometer.h
 * Header file for the accelerometer functions.
 */
...
#include <Arduino.h>
#include <math.h>  // needed for floating point stuff.
...
extern const byte AccelScale;  // Sets full-scale range to +/-2, 4, or 8g. Used to calc real g values.
/* Set the output data rate below. Value should be between 0 and 7*/
extern const byte AccelDataRate;  // 0=800Hz, 1=400, 2=200, 3=100, 4=50, 5=12.5, 6=6.25, 7=1.56
/* Pin definitions */
extern const int AccelInt1Pin;  // These can be changed, 2 and 3 are the Arduinos ext int pins
extern const int AccelInt2Pin;

extern byte accelData[6];  // x/y/z accel register data store here
extern int accelCount[3];  // Stores the 12-bit signed value
extern float accelValue[3];  // Stores the real accel value in g's

//variables for accerometer 
extern unsigned int accelSampleX,accelSampleY,accelSampleZ;
extern unsigned long accelStateX,accelStateY,accelStateZ;
...
Dooes not do any noise filtering.
